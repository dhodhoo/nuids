...
const AI_API_URL =
  import.meta.env.VITE_AI_API_URL || "/backend/api/chat/custom";
const AI_API_KEY = import.meta.env.VITE_AI_API_KEY || "";

async function callAi(conversation) {
  const isDirect = AI_API_URL.startsWith("http") && !AI_API_URL.includes("/backend/");
  const token = isDirect ? AI_API_KEY : getToken();
  const res = await fetch(AI_API_URL, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: "Bearer " + token },
    body: JSON.stringify(
      isDirect
        ? { feature: "custom", systemPrompt: SYSTEM_PROMPT, conversation }
        : { systemPrompt: SYSTEM_PROMPT, conversation },
    ),
  });
  const json = await res.json();
  if (!json.success)
    throw new Error(json.message || json.error || "AI gagal merespon");
  const reply = json.data?.reply || json.reply;
  if (!reply) throw new Error("AI gagal merespon");
  return reply;
}
...
export default function AIChatPage() {
  ...
  async function handleSend(text) {
    ...
    try {
      ...
      const saved = await api.chat.send({
        child_id: child.id,
        role: "ai",
        message: replyText,
      });
  ...
  return (
    <PageShell>
      <PageHeader
        title="Nuids AI"
        subtitle="Asisten pertumbuhan anak"
        showBack
      />
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
              ...
              historyMessages
                ...
                .map((m) => (
                  <div
                    key={m.id}
                    onClick={recoverChat}
                    className="mb-2 pb-2 border-b border-gray-50 last:border-0 cursor-pointer hover:bg-gray-50 rounded-lg p-2 -mx-2 transition-colors"
                  >
        ...
        {messages.length === 0 && !showHistory ? (
          <p className="text-center text-text-muted text-[13px] mt-8">
            Belum ada pesan. Mulai bertanya!
          </p>
        ) : (
          !showHistory &&
        ...
        {sending && (
          <div className="flex items-end gap-2">
            ...
            <div className="bg-white rounded-2xl rounded-bl-sm px-4 py-3 shadow-sm">
              <span className="text-[13px] text-text-muted animate-pulse">
                Mengetik...
              </span>
            </div>
          </div>
